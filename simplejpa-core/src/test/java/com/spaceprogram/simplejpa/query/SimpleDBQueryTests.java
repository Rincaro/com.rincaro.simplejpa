...
public class SimpleDBQueryTests extends UnitilsJUnit4 {
    ...
    public void testNoParameterReplacement() {
        this.mockEM.returns(this.mockEMFactory.getMock()).getFactory();
        this.mockEM.returns(MyTestObject.class).ensureClassIsEntity("MyTestObject");
        this.mockEMFactory.returns("simplddbquerytests").getPersistenceUnitName();

        SimpleDBQuery q = new SimpleDBQuery(this.mockEM.getMock(), "select count(*) from MyTestObject");
    ...
    public void testSimpleParameterReplacement() {
        ...
        SimpleDBQuery q = new SimpleDBQuery(this.mockEM.getMock(), "select count(*) from MyTestObject where col1 = :val1");
    ...
    public void testMultipleReplacements() {
        ...
        SimpleDBQuery q = new SimpleDBQuery(this.mockEM.getMock(), "select * from MyTestObject where col1 = :val1 and col10 > :val10");
        q.setParameter("val1", now);
        q.setParameter("val10", 10);
        AmazonQueryString aq = q.createAmazonQuery(false);
        assertFalse(aq.isCount());
        String expected = new StringBuilder().append("select * from `simplddbquerytests-MyTestObject` where col1 = '").append(AmazonSimpleDBUtil.encodeDate(now)).append("' and col10 > '09223372036854775818'").toString();
    ...
    public void testCollectionValueReplacement() {
        ...
        SimpleDBQuery q = new SimpleDBQuery(this.mockEM.getMock(), "select count(*) from MyTestObject where col1 in (:val1)");
        q.setParameter("val1", param);
        AmazonQueryString aq = q.createAmazonQuery(false);
        assertTrue(aq.isCount());
        String convertedDate = AmazonSimpleDBUtil.encodeDate(now);
        String convertedNum = "09223372036854775818";
        assertEquals(String.format("select count(*) from `simplddbquerytests-MyTestObject` where col1 in ('%s', '%s')", convertedDate, convertedNum), aq.getValue());
    }
    ...
    public void testPositionalParameterReplacement() {
        ...
        SimpleDBQuery q = new SimpleDBQuery(this.mockEM.getMock(), "select * from MyTestObject where col1 = ?1 and col10 > ?2");
